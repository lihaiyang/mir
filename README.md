...
- 发现新版本后自动在后台下载 `.dmg` 安装包，右下角浮窗显示下载进度
- 下载完成自动打开安装包，用户只需把 MIR 拖入「应用程序」文件夹即完成更新（覆盖旧版）

该方案不依赖代码签名，也不走 Squirrel 签名校验链路，因此即使未签名的构建也能正常自动更新。更新器仅在打包后的 macOS 应用中生效（开发模式下不运行）。
...
1. 修改 `package.json` 与 `package-lock.json` 中的 `version` 字段
2. 提交到 `main` 分支
3. 打 tag 并推送：`git tag v0.1.x && git push origin v0.1.x`
4. 工作流自动在 macOS（dmg）和 Windows（exe）上构建，创建 GitHub Release 并上传安装包

已有的 macOS 用户会在应用启动后自动检测到新版本并下载更新。
